 ...
 *
 * @param {String} the group ID (32 char, lower case, hex values (UUID without hashes)
...
function _addGroup(groupId){
	return new Promise(resolve => {
		addGroup(groupId, (err,auth_key,linux_gid) =>{
			resolve({err,auth_key,linux_gid});
		});
	});
}
 ...
 * @return none
 */
async function addGroupAsync(groupId) {
	return await _addGroup(groupId);
}
...
function _removeGroup(groupId){
	return new Promise(resolve => {
		removeGroup(groupId, (err,cmd113) =>{
			resolve({err,cmd113});
		});
	});
}
...
async function removeGroupAsync(groupId) {
	return await _removeGroup(groupId);
}
