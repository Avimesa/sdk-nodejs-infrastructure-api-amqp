...
/**
 * Consumes for accounting records
 *
 * Creates an exclusive connection retrieving accounting records.
 *
 * @param {Function} cb(err, msg, ack): called on response
 *
 * @return none
 */
function acctRecordConsume(cb) {
	rmq.exclusiveSubscribe("dc_acct_q",  function (err, msg, ack) {
		if(err){
			cb(true, "", null);
		}
		else {
...
module.exports.acctRecordListener = acctRecordConsume;
